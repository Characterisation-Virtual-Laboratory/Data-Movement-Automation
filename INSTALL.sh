...
# Update service file
sed -i "s|^Wants=.*|Wants=${SCRIPT_NAME}.timer|g" ${SYSTEMD_PATH}/${SCRIPT_NAME}.service
...
# Update timer file
sed -i "s|^Requires=.*|Requires=${SCRIPT_NAME}.service|g" ${SYSTEMD_PATH}/${SCRIPT_NAME}.timer
